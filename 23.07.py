...
logging.basicConfig(level=logging.DEBUG, filename='optimuslatest1.log', filemode='a', format='%(message)s')
...
class PoliticalSystem:
    ...
    def generate_norms(self):
        ...
        if new_norm.id not in self.norm_ids:
        ...
        if self.has_decision():
            ...
            for _ in range(2):
                ...
                if new_norm.id not in self.norm_ids:
                    ...
                    print(f"PoliticalSystem: Created new norm from the decision {new_norm.text}")
            return new_norms
        return []
...
class Government(PoliticalSystem):
    def create_norm(self):
        ...
        self.regulation_counter += 1  # Increment regulation counter here
        return regulation
    ...
    def make_regulations(self, citizen_pressure):
        ...
        if citizen_pressure > 3 and self.norm_pool:
            ...
            regulation_a = Regulation(self.regulation_counter, f"Regulation {self.regulation_counter} based on Law {norm.id}", True, random.randint(COMPLEXITY_MIN, COMPLEXITY_MAX))
            regulation_b = Regulation(self.regulation_counter + 1, f"Regulation {self.regulation_counter + 1} based on Law {norm.id}", True, random.randint(COMPLEXITY_MIN, COMPLEXITY_MAX))
...
class JudicialSystem:
    def __init__(self, parliament, government):
        self.case_pool = []
        self.case_counter = 0
        self.case_ids = set()
    ...
    def initialize_courts(self):
        # Remove these initializations as they are now handled in the constructor
        pass
...
judicial_system = JudicialSystem(parliament, government)
judicial_system.initialize_courts()
...
class Society:
    ...
    def __init__(self, use_fixed_seed=True):
        ...
        self.parliament = Parliament()
        self.government = Government()
        self.judicial_system = JudicialSystem(self.parliament, self.government)
